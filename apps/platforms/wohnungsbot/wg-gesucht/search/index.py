...
def fetch_csrf_token(session, max_retries=3, wait=2):
    """Attempts to fetch the CSRF token with retries."""
    csrf_token = None
...
def search_listings(session, filters):
    ...
    # Convert move-in dates to timestamps if available
    ...
    for listing in soup.find_all("div", class_="offer_list_item"):
        ...
        print(f"Room Size: {room_size} m² (Extracted from '{room_size_text}')")

        # Extract and split location details
        ...
        print(f"Availability Start: {start_date_text}, Availability End: {end_date_text}")

        # Calculate and filter based on stay_days
        stay_days = None
        if start_date_text and end_date_text:
            try:
                ...
                if stay_days < min_stay_days:
                    print(f"Skipping listing '{title}' due to min_stay_days filter of {min_stay_days}")
                    continue
                print(f"Calculated stay_days: {stay_days}")
            except ValueError:
                print(f"Could not parse end date for listing '{title}', skipping stay_days filter.")
                continue

        # Extract ad person (lister name)
        ad_person_element = listing.find("span", class_="ml5")
        ad_person = ad_person_element.text.strip() if ad_person_element else "Unknown"
        print(f"Lister Name: {ad_person}")

        # Append valid listing with extracted details
        listings.append({
            "title": title,
            "link": link,
            "rent": rent,
